/* eslint-disable react/sort-comp,import/no-extraneous-dependencies */
...
import { TransitionContext, RouteDirection } from './Types/index';
import * as Constants from './TransitionConstants';

import { initTransitionTypes, getCarouselTransitionElements } from './CarouselTransitions/index';
import { initInterpolatorTypes, getSharedElements } from './CarouselInterpolators/index';
...
class TransitionOverlayView extends React.Component<Props> {
  ...
  render() {
    ...
    const transitionElements = this.props.transitionElements ? this.props.transitionElements
      .filter(i =>
        i.route === currentRoute || i.route === leftRoute || i.route === rightRoute) : [];
    ...
    const transitionViews = getCarouselTransitionElements(
      transitionElements,
      transitionContext,
      progressIndex,
      this.props.navigation,
    );
    const sharedElementViews = getSharedElements(
      this.props.sharedElements,
      this.getInterpolation, progressIndex, this.props.navigation, getTransitionProgress,
    );
  ...
  getVisibilityStyle() {
    ...
    const inputRange = [
      ...
      (index) - Constants.OP, index, index + Constants.OP,
      (index + 1) - Constants.OP,
      index + 1,
    ];
    ...
    if (this.props.sharedElements) {
      this.props.sharedElements.forEach((pair) => {
        ...
        if (pair.fromItem && pair.fromItem.anchors) {
          for (let n = 0; n < pair.fromItem.anchors.length; n += 1) {
        ...
        if (pair.toItem && pair.toItem.anchors) {
          for (let n = 0; n < pair.toItem.anchors.length; n += 1) {
  ...
  getTransitionContext(transitionElements): TransitionContext {
    const {
      getDirectionForRoute, getIndex, getDirection,
      getRoutes, getTransitionProgress
    } = this.context;
    ...
    const delayIndexTo = 0;
    const delayFromFactor = 1;
    const delayToFactor = 1;
    const getDirectionForRouteName = routeName => getDirectionRouteName(
      routeName,
      { left: leftRoute, right: rightRoute, curr: currentRoute },
    );
    return {
      delayCountLeft,
      delayCountCurrent,
