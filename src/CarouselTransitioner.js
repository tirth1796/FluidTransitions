...
import { StyleSheet, Easing, I18nManager, Animated, Dimensions, PanResponder, InteractionManager } from 'react-native';
...
class FluidTransitioner extends React.Component<*> {
    ...
    animation.start(({ finished }) => {
      if (finished) {
        ...
        if (!this._isResponding && backFromScene) {
          this.props.navigation.navigate(this.props.navigation.state.routes[toValue].routeName);
        }
      }
    });
    this._animations.push(animation);
  ...
  _render = (props) => {
    ...
    const fromRoute = index > 0 ? props.scenes[index - 1].route.routeName : null;
    ...
    const scenes = props.scenes.map(s => this._renderScene({ ...props, scene: s }));
