...
import { createAnimatedWrapper, mergeStyles } from '../Utils/index';
import { InterpolatorSpecification, } from '../Types/index';
...
const getTransitionStyle = (fromItem: TransitionItem, toItem: TransitionItem, getInterpolationFunction: Function, index, navigation) => {
  const interpolatorInfo: InterpolatorSpecification = {
    ...
    index: getIndex(fromItem, navigation) - index,
  };
  ...
  getInterpolatorTypes().forEach((interpolator) => {
    ...
    if (interpolatorResult) {
      if (interpolatorResult.nativeAnimationStyles) { nativeStyles.push(interpolatorResult.nativeAnimationStyles); }
      if (interpolatorResult.animationStyles) { styles.push(interpolatorResult.animationStyles); }
    }
  });
  return {
    nativeStyles: {
...
const getSharedElements = (sharedElements: Array<any>, getInterpolationFunction: Function, index, navigation, getTransitionProgress) => sharedElements.map((pair, idx) => {
