/* eslint-disable no-await-in-loop,react/sort-comp,class-methods-use-this,import/no-extraneous-dependencies */
...
export default class TransitionItemsView extends React.Component<
  ...
  componentDidMount() {
    ...
    const te = this._transitionItems.getTransitionElementsCarousel(getConsecutiveRoutes(
  ...
  componentDidUpdate() {
    ...
    let measureAndUpdateFunc = async () => {
      const consecutiveRoutes = getConsecutiveRoutes(
        this.props.navigation.state.index,
        this.props.navigation.state.routes,
      );
      const sharedElements = this._transitionItems.getSharedElementsCarousel(...consecutiveRoutes);
  ...
  async measureItems(
    ...
    if (sharedElements) {
      for (let i = 0; i < sharedElements.length; i += 1) {
