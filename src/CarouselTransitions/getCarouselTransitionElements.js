/* eslint-disable no-param-reassign,no-use-before-define,import/no-extraneous-dependencies */
...
const getDelays = (routeDirection, transitionContext) => {
  ...
  return { delayCount: delayCount + 1, delayIndex };
};

const getCarouselTransitionElements = (transitionElements: Array, transitionContext: TransitionContext, currentIndex, navigation) => transitionElements.map((item, idx) => {
...
const getPositionStyle = (item, delayCount: number, delayIndex: number, transitionContext: TransitionContext, currentIndex, navigation) => ({
  // borderWidth: 1,
  // borderColor: '#00FFFF',
...
const getTransitionStyle = (item, delayCount: number, delayIndex: number, transitionContext: TransitionContext, currentIndex, navigation) => {
  ...
  if (progress) {
    ...
    if (transitionFunction) {
      ...
      const end = Constants.TRANSITION_PROGRESS_END;
...
const getTransitionFunction = (item) => {
